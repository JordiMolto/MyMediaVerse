<template>
  <div class="media-view">
    <header class="view-header">
      <h2>Animes</h2>
      <button @click="openForm()" class="action-button">Añadir Anime</button>
    </header>

    <div class="controls-bar">
      <div class="filter-group">
        <label for="filter-estado-animes">Filtrar por Estado:</label>
        <select id="filter-estado-animes" v-model="filterEstado">
      ...
      <div class="filter-group">
        <label for="filter-favorito-animes">
          <input type="checkbox" id="filter-favorito-animes" v-model="filterFavorito">
          Mostrar solo Favoritos
        </label>
      </div>
      <div class="sort-group">
        <label for="sort-by-animes">Ordenar por:</label>
        <select id="sort-by-animes" v-model="sortBy">
    ...
    <div v-if="showForm" class="form-modal">
      <div class="form-content">
        <h3>{{ currentItem ? 'Editar' : 'Añadir' }} Anime</h3>
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            <label for="titulo-anime">Título:</label>
            <input type="text" id="titulo-anime" v-model="formValues.titulo" required>
          </div>
          <div class="form-group">
            <label for="estado-anime">Estado:</label>
            <select id="estado-anime" v-model="formValues.estado">
              <option value="pendiente">Pendiente</option>
              <option value="viendo">Viendo</option>
              <option value="completado">Completado</option>
              <option value="droppeado">Droppeado</option>
          ...
          <div class="form-group">
            <label for="puntuacion-anime">Puntuación (1-10):</label>
            <input type="number" id="puntuacion-anime" v-model.number="formValues.puntuacion" min="1" max="10">
          </div>
          <div class="form-group">
            <label for="nota-anime">Nota:</label>
            <textarea id="nota-anime" v-model="formValues.nota"></textarea>
          </div>
          <div class="form-group">
            <label for="imagenUrl-anime">URL de la Imagen:</label>
            <input type="text" id="imagenUrl-anime" v-model="formValues.imagenUrl">
          </div>
          <div class="form-group checkbox-group">
            <input type="checkbox" id="favorito-anime" v-model="formValues.favorito">
            <label for="favorito-anime">Marcar como Favorito</label>
          </div>
    ...
    <div class="media-table">
      ...
      <div v-if="filteredAndSortedItems.length === 0" class="empty-state-row">
        <p>No hay animes que coincidan con los filtros actuales.</p>
      </div>
      <template v-else>
        <MediaListItem 
...
const loadMockData = () => {
  allItems.value = [
    { id: 1, titulo: 'Attack on Titan', tipo: 'anime', estado: 'completado', puntuacion: 10, nota: 'Épico.', favorito: true, imagenUrl: 'https://image.tmdb.org/t/p/w500/hTP1DtLGFamjfu8WqjnuQdP1n4i.jpg' },
    { id: 2, titulo: 'Jujutsu Kaisen', tipo: 'anime', estado: 'viendo', puntuacion: 9, favorito: true, imagenUrl: 'https://image.tmdb.org/t/p/w500/fRabXleLhsM2xL2PjXkYqZJ9m5q.jpg' },
    { id: 3, titulo: 'Demon Slayer', tipo: 'anime', estado: 'pendiente', favorito: false, imagenUrl: 'https://image.tmdb.org/t/p/w500/xUfRZu2mi8jH6j_QfYStgvq1c3b.jpg' },
    { id: 4, titulo: 'Spy x Family', tipo: 'anime', estado: 'completado', puntuacion: 8, favorito: false, imagenUrl: 'https://image.tmdb.org/t/p/w500/3r4LYFuX9LpBlG1xGjLPTQf0fEt.jpg' },
  ];
};
